...
class TrelloCardsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
          ...
          if @trello_card.save
            ...
            logger.error "Unable to save card "
            @trello_card.errors.each do |attribute, message|
                  logger.error "#{attribute} --- #{message} "
            end 
  ...
  private
    ...
    def trello_card_params
      params.require(:trello_card).permit(:name, :description, :environment, :priority , :username , :hostname , :message_type , :datetime)
    end
end
